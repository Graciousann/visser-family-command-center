...
function familyCode(){ return (localStorage.getItem(CODE_KEY) || '').trim(); }
function encodedFamilyCode(){ return encodeURIComponent(familyCode()); }
function syncDetails(message, kind=''){
  const el=document.querySelector('#syncDetails');
  if(!el) return;
  el.textContent=message;
  el.className=`sync-detail ${kind}`.trim();
}
...
async function apiJson(url, options={}){
  ...
  if(!res.ok){
    const detail=data?.error || data?.message || data?.raw || res.statusText || 'Request failed';
...
async function checkHealth(){
  return apiJson('/api/health',{cache:'no-store'});
}
async function pullCloud(){
  if(!familyCode()) { setSync(false,'Local'); syncDetails('Enter the family code to connect shared sync.'); return; }
  try{
    setSync(false,'Syncing…');
    syncDetails('Checking Cloudflare connection…');
    const health=await checkHealth();
    if(!health?.dbBound) throw new Error('Cloudflare cannot see the DB binding.');
    if(!health?.tokenConfigured) throw new Error('Cloudflare cannot see HOUSEHOLD_TOKEN.');
    if(!health?.databaseReady) throw new Error(`Database check failed${health?.databaseError?`: ${health.databaseError}`:''}`);
    const remote=await apiJson('/api/state',{headers:{'X-Household-Code':encodedFamilyCode()},cache:'no-store'});
    ...
    syncDetails(remote?.state ? 'Connected to shared family data.' : 'Connected. Shared database is ready; this device will create the first shared copy when you make a change.','success');
  }catch(e){
    cloudConnected=false; setSync(false,'Local');
    console.error('Shared sync failed',e);
    syncDetails(`Could not connect: ${e.message}`,'error');
...
async function pushCloud(){
  ...
  try{
    const res=await apiJson('/api/state',{method:'PUT',headers:{'Content-Type':'application/json','X-Household-Code':encodedFamilyCode()},body:JSON.stringify({state}),cache:'no-store'});
    cloudConnected=true; setSync(true,'Shared');
    syncDetails('Saved to shared family data.','success');
    return res;
  }catch(e){
    cloudConnected=false; setSync(false,'Local');
    console.error('Shared save failed',e);
    syncDetails(`Shared save failed: ${e.message}`,'error');
  }
}
...
function render(){
  ...
  document.querySelector('#todayDateLabel').textContent=todayLabel();
  document.querySelector('#todayEvents').innerHTML = state.events.filter(e=>e.day===todayName()).map(eventHTML).join('') || '<div class="empty">Nothing added yet.</div>';
...
document.querySelector('#saveCode').addEventListener('click',()=>{const code=document.querySelector('#familyCode').value.trim();localStorage.setItem(CODE_KEY,code);pullCloud();});
